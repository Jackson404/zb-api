<template>
	<view>
		<view style="position: fixed;top: 0;left: 0;right: 0;z-index: 9999999;">
			...
			<image @tap="go1" src="https://521.zhengbu121.com/statics/images/321123.jpg"  style="width: 100%;display: block;height: 340upx;"></image>
			<!-- 搜索想 -->
				  ...
				  <view class="grace-filter" id="grace-filter-header">
				  	...
				  	<view class="items" @tap='showOptions2'>薪资<text class="grace-iconfont icon-arrow-down"></text></view>
					<view class="items" @tap='showOptions3'>要求<text class="grace-iconfont icon-arrow-down"></text></view>
				  ...
				  <!-- 	<view class="items" @tap='changePriceOrder'>
				  	...
				  	<view class="items" @tap='showOptions99'>其他<text class="grace-iconfont icon-arrow-down"></text></view>
				  	<!-- 第一个选项 -->
				  	<view class='grace-filter-options'  v-if="showingIndex == 1" :style="{'width':'100%', 'height' : filterHeight, 'padding':'0','background':'rgba(0,0,0,0.5)'}">
				  		<view style="background: #fff;display: flex;;flex-wrap: nowrap;overflow: scroll;" :style="{'height':filterHeight1}">
							<view style="width: 25%;box-sizing: border-box;background: #F6F6F6;height: 100%;overflow: scroll;">
								<view @tap.stop="chnn(index)" style="height: 100upx;line-height: 100upx;text-align: center;color: #666;font-size: 28upx;" :class="item.checked ?'xx':''" v-for="(item,index) in hangye" :key="index">
									{{item.name}}
								</view>
							</view>
	...
	export default {
		...
		methods:{
			...
			hangye1:function(e){
				...
				uni.request({
					url: _this.apiServer + '/api/v1.CategoryManagement/getAllByTree',
					method: 'POST',
					header: {'content-type' : "application/x-www-form-urlencoded"},
					data:{
			...
			go:function(){
				...
				uni.navigateTo({
					url: '../../pages/index/seach?con='+this.con
				});
			},
			// 上拉刷新
			getdata(){
